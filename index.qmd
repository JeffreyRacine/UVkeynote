...
subtitle: XIII FORO INTERNACIONAL DE ESTADÍSTICA APLICADA
author:
...
format:
  revealjs:
    ...
    footer: "Locally Adaptive Online FDA | V. Patilea & J. Racine | jeffreyracine.github.io/UVkeynote"
...
- Link to slides - <a href="https://jeffreyracine.github.io/UVkeynote">jeffreyracine.github.io/UVkeynote</a> (case sensitive)

- Spanish Translation Available - FIRST go to the link above in your browser, then advance to this slide and click here: <a href="https://jeffreyracine-github-io.translate.goog/UVkeynote/?_x_tr_sl=auto&_x_tr_tl=en&_x_tr_hl=en&_x_tr_pto=wapp#/title-slide">Google Translate</a>, then at the top of the page change *English detected -> English* to *English detected -> **Spanish** *
...
-   However, *we never know the curve values at all points*

- The curves are *only available at discrete points* (e.g., $(Y^{(i)}_m , T^{(i)}_m) \in\mathbb R \times [0,1]$)

- As well, the points at which curves are available can *differ across curves*

- Additionally, the curves may be *measured with error*
...
-   Our method adapts, *pointwise*, to
    ...
    -   the *local regularity* of the underlying process (i.e., the Hölder exponent $H_t$ defined below) and to *measurement noise* (i.e., we are able to separate *noise* from *regularity* thanks to *replication*)

    -   the *purpose(s)* of estimation (i.e., $\mu(t)$ versus $\Gamma(s,t)$ defined below)
...
-   We are also concerned with computational limitations frequently encountered in this framework (we shall adopt a recursive solution)
...
-   Essentially, there is prior information contained in the $N-1$ sample curves that can be exploited to learn about the $N$th sample curve, which is *not* available in, say, classical regression analysis
...
-   We propose simple estimates for *local regularity* and demonstrate that real data may be irregular
...
- *minimax optimal rates for mean and covariance functions*,  [@lepski_mammen_spokoiny:1997;@cai_yuan:2011], Cai & Yuam (2010, 11, 12) (see minute 23 of zoom video feb 28 2024)
...
-   The idea is to estimate $\theta(t_1,t_3)$ and $\theta(t_1,t_2)$ averaging over curves
...
-   $\sigma_t^2$ is estimated using one-half the squared differences of the two closest $Y^{(i)}$ observations at $t$, averaged across all curves (see Appendix)
...
-   In order to estimate $\mu(t)$ at point $t$ and $\Gamma(s,t)$ at points $s$ and $t$, ideally we would use the (unknown, continuous) curves evaluated at these points, hence the *ideal* estimators given $N$ curves would be 
    \begin{align*}
...
## Online Recursive Updating of $\widehat X^{(i+1)}(t)$, $\widehat\mu(t)$, $\widehat\Gamma(s,t)$
...
- NB - our $L$ controls the signal-to-noise ratio ($L$-to-$\sigma$ ratio). A larger $L$, other things equal, means the signal is very informative as the amplitude of the oscillations is very large relative to the measurement error (around 56 mins into Mar 7 2024 video)
